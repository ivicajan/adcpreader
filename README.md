[![PyPI version](https://badgen.net/pypi/v/adcpreader)](https://pypi.org/project/adcpreader)
[![Docs badge](https://readthedocs.org/projects/adcpreader/badge/?version=latest)](https://adcpreader.readthedocs.io/en/latest/)
...
use `sink.keys()` to list all variables that are accessible
...
a list of filenames as argument to `pipeline.process`. However,
...
processed when supplied as a list to `pipeline.process()`, or
the pipeline is defined again.

A third way (not recommended), is to leave the coroutines open, by
supplying the optional keyword `close_coroutines_at_exit=False`. Then it is the user's
...
This will have no effect on the contents of `sink`, but it will
